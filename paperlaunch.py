...
def paperlaunch(name='paperlaunch-project', github=False, private=True):
    """
...
def create_repo_local(name):
    git.Repo.init(path=join(curdir, name), mkdir=True)
...
def create_repo_github(name, private=True):
    ...
    if org is None:
        ...
        actor = org
    remote_repo = actor.create_repo(name, description="Description", 
                                    gitignore_template="TeX", private=private)
...
def main():
    ...
    parser=ArgumentParser(doc="Set up a new LaTeX project directory")

    parser.add_argument('name', type=str,
                        help="Project name. Used as folder name "
                             "and repo address.")
    parser.add_argument('--public', type='store_true',
                        help="Don't make the repository private")
    parser.add_argument('--github', '--gh', type='store_true',
